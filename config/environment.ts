const envConfig = {
  ...
  MQTT_PORT_SSL:
    typeof process.env.EXPO_PUBLIC_MQTT_PORT_SSL === "string"
      ...
      : 0,
  MQTT_VERSION:
    typeof process.env.EXPO_PUBLIC_MQTT_VERSION === "string"
      ? parseInt(process.env.EXPO_PUBLIC_MQTT_VERSION)
      : 0,
  MQTT_QOS:
    typeof process.env.EXPO_PUBLIC_MQTT_QOS === "string"
      ...
      : 0,
  MQTT_USERNAME:
    (process.env.EXPO_PUBLIC_MQTT_USERNAME ?? "").length === 0
      ? undefined
      : process.env.EXPO_PUBLIC_MQTT_USERNAME,
  MQTT_PASSWORD:
    (process.env.EXPO_PUBLIC_MQTT_PASSWORD ?? "").length === 0
      ? undefined
      : process.env.EXPO_PUBLIC_MQTT_PASSWORD,
  MQTT_TOPICS:
    typeof process.env.EXPO_PUBLIC_MQTT_TOPICS === "string"
      ? process.env.EXPO_PUBLIC_MQTT_TOPICS.split(",")
      : [],
